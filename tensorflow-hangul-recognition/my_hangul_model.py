...
# Default paths.
MY_LABEL_FILE = os.path.join(SCRIPT_PATH, './labels/한국어 어휘 목록.txt')
MY_TFRECORDS_DIR = os.path.join(SCRIPT_PATH, 'my-tfrecords-output')
MY_OUTPUT_DIR = os.path.join(SCRIPT_PATH, 'my-saved-model')

MODEL_NAME = 'my_hangul_tensorflow'
IMAGE_WIDTH = 64
IMAGE_HEIGHT = 64
...
def my_export_model(model_output_dir, input_node_names, output_node_name):
...
def my_weight_variable(shape):
    """Generates a weight variable of a given shape."""
    initial = tf.random.truncated_normal(shape, stddev=0.1)
    return tf.Variable(initial, name='weight')


def my_bias_variable(shape):
...
def main(label_file, tfrecords_dir, model_output_dir, num_train_epochs):
    ...
    W_conv1 = my_weight_variable([5, 5, 1, 32])
    b_conv1 = my_bias_variable([32])
    ...
    W_conv2 = my_weight_variable([5, 5, 32, 64])
    b_conv2 = my_bias_variable([64])
    ...
    W_conv3 = my_weight_variable([3, 3, 64, 128])
    b_conv3 = my_bias_variable([128])
    ...
    W_fc1 = my_weight_variable([8*8*128, 1024])
    b_fc1 = my_bias_variable([1024])
    ...
    W_fc2 = my_weight_variable([1024, num_classes])
    b_fc2 = my_bias_variable([num_classes])
    ...
    with tf.Session() as sess:
        ...
        my_export_model(model_output_dir, [input_node_name, keep_prob_node_name],
                     output_node_name)
